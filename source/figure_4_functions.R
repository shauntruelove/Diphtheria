...
# font_import()
# library(showtext)
# font_add("Myriad Pro", "C:/Users/shaun/Downloads/MyriadPro-Regular.otf")
# showtext_auto()
...
plot_Vc_curve_plot_data_untreated <- function(restrict_quantiles=c(0,1)){
  ...
  ggplot() + 
    ...
    theme(axis.title.x=element_blank(), axis.title.y=element_text(face="italic")) + 
...
plot_Vc_curve_plot_data_treated <- function(delay=1, alpha=.9){
...
plot_R0_density <- function(delay=1, alpha=0.9, alpha_density=0.5, blank=FALSE){
...
plot_combined_VcR0 <- function(delay=1, alpha=0, 
                               alpha_density_R0=0.5){
  plot_grid(
    # Top Plot
    if(alpha==0){
      plot_Vc_curve_plot_data_untreated()
    }else{
      plot_Vc_curve_plot_data_treated(delay=delay, alpha=alpha)
    },
    # Bottom Plot
...
blankplot1_fn <- function(restrict_quantiles = c(0,1), alpha_plot=0.35){
  # Source code to adjust Vc for treatment
  source('source/vc_treat.R')
  
  # Calculate Treated Vc
  data_<- vc_treat(delay = 1, alpha = 0, restrict_quantiles = c(0,1))
  ...
  r0_iqr <- get_R0_IQR(delay=1, alpha=0, restrict_quantiles=restrict_quantiles)
    ...
    ggplot(KnGT_R0_df) +
        ...
        theme(axis.text.y=element_text(colour = "white"), axis.title.y=element_text(colour = "white"),
              axis.ticks.y=element_blank(),  axis.line.y= element_blank(),axis.ticks.x=element_blank(),
              axis.title.x=element_text(colour = "white"), axis.line.x= element_blank(),axis.text.x=element_text(colour = "white"))
}

blankplot2_fn <- function(restrict_quantiles = c(0,1)){
