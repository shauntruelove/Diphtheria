...
ret_known_fixCov <- stanc(fileName_knownGT_fixedCoV) # Check Stan file
ret_sm_knownGT_fixCov <- stan_model(stanc_ret = ret_known_fixCov) # Compile Stan code
save(fileName_knownGT_fixedCoV, ret_known_fixCov, ret_sm_knownGT_fixCov, file="source/R0stan_knownGT_fixedCoV_compiled.RData")
...
fit_knGT_fixCoV <- sampling(ret_sm_knownGT_fixCov, warmup=100, iter=200, seed=123, data=full_data_knGT, chains=4, control=list(adapt_delta=.85))
fit_knGT_fixCoV

fit_knGT_fixCoV_noRoh <- sampling(ret_sm_knownGT_fixCov, warmup=100, iter=200, seed=123, data=full_data_knGT_noRoh, chains=4, control=list(adapt_delta=.85))
...
# Save STAN results
save(ret_sm_knownGT, fit_knGT, full_data_knGT, file='results/R0res_knownGT_200iters.RData')
