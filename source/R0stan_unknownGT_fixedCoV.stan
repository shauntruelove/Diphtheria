...
data {
  ...
  real<lower=0, upper=1> Vk_cov[3];          // Coef of Variation - inputted as percent.
...
parameters {
  ...
  real<lower=0> shape;                      // Shape parameter for Generation Time gamma distribution [FIT]
  real<lower=0> scale;                      // Scale of Generation Time gamma distribution [FIT]
...
model { 
  ...
  // Calculate p vector -- the generation time probability distribution
      for (m in 1:M) {
        for (j in 1:T_m1[m]) {
          p[m,j] = gamma_cdf(j, shape, 1/scale) - gamma_cdf(j-1, shape, 1/scale);  
        }
      }
