...
data {
  ...
  matrix[M,3] Vk_pop;                       // proportion of pop receiving k doses in each outbreak
  matrix[M,3] Vk_cases;                     // proportion of cases receiving k doses in each outbreak
  int<lower=0, upper=1> use_schick[M];      // Logical, if outbreak occurred during prevaccination era (pre-1923)
  int<lower=0, upper=1> est_Vs[M];       // If stan should be allowed to sample Vs for population
  int<lower=0, upper=1> est_Vs_from_cases[M];       // If stan should estimate Vs for population from cases
...
transformed parameters {
  ...
  matrix<lower=0,upper=1>[M,3] Vk_pop_est;
  ...
  // Estimate population vaccination coverage from case coverage, where data are available 
  for (m in 1:M){
    if (est_Vs_from_cases[m]==1){
      Vk_pop_est[m,3] = Vk_pop[m,3];
      Vk_pop_est[m,2] = (pk[1] / (pk[2]/Vk_cases[m,2] - pk[2] + pk[1])) * (1 - Vk_pop_est[m,3]);
      Vk_pop_est[m,1] = 1 - (Vk_pop_est[m,3] + Vk_pop_est[m,2]);
    } else {
      Vk_pop_est[m,3] = Vk_pop[m,3];
      Vk_pop_est[m,1] = Vk_pop[m,1];
      Vk_pop_est[m,2] = Vk_pop[m,2];
    }
  }
  ...
  for (m in 1:M) { 
    R[m] = 0;
    if (use_schick[m]==1){
      R[m] = Rs[m]*(V_est[m,3] + (1-V_est[m,3])*tau);
    } else {
      R[m] = Rs[m]*(V_est[m,1]*(pk[1] + (1-pk[1])*tau) + 
                    V_est[m,2]*(pk[2] + (1-pk[2])*tau) + 
                    V_est[m,3]*(pk[3] + (1-pk[3])*tau));
    }
  }
...
model { 
  ...
  // Estimate global log Rs_star, with random effect (sigma), from log Rs
  lRs ~ normal(lRs_star, sigma);
  
  // Estimate the Vaccination Coverages for each population
  for (m in 1:M){
    V_est[m,3] ~ normal(Vk_pop_est[m,3], V3_sd[m]);
    V_est[m,1] ~ normal(Vk_pop_est[m,1], V1_sd[m]);
  }
  // for (m in 1:M){
  //   V_est[m,3] ~ normal(Vk_pop[m,3], Vk_pop[m,3]*Vk_cov[3]);
  //   V_est[m,1] ~ normal(Vk_pop[m,1], Vk_pop[m,1]*Vk_cov[1]);
  // }
  
  // R0 Adjustment parameters
  //tau ~ normal(logtau_mean, tau_sd);
  logtau ~ normal(logtau_mean, logtau_sd);

  //target += normal_lpdf(logtau | logtau_mean, logtau_sd);
