...
full_data_stoch[['logtau_sd']] <- 0.285
...
full_data_NoATB <- restrict_stan_input_data(data=full_data, outbreaks_remove=which(full_data$NoATB==0))
full_data_knGT_NoATB <- restrict_stan_input_data(data=full_data_knGT, outbreaks_remove=which(full_data_knGT$NoATB==0))

full_data_ATB <- restrict_stan_input_data(data=full_data, outbreaks_remove=which(full_data$NoATB==1))
full_data_knGT_ATB <- restrict_stan_input_data(data=full_data_knGT, outbreaks_remove=which(full_data_knGT$NoATB==1))
...
# Known GT model
#fileName_knownGT <- "./source/R0_joint_est_full_withR0adjust_knownGT-TEST.stan"
...
fit_knGT_NoATB <- sampling(ret_sm_knownGT, warmup=100, iter=200, seed=123, data=full_data_knGT_NoATB, chains=4, control=list(adapt_delta=.85))
fit_knGT_NoATB

fit_knGT_ATB <- sampling(ret_sm_knownGT, warmup=100, iter=200, seed=123, data=full_data_knGT_ATB, chains=4, control=list(adapt_delta=.85))
fit_knGT_ATB

# Save STAN results
save(ret_known, fit_knGT, fit_knGT_noRoh, full_data_knGT, full_data_knGT_noRoh, file='./results/R0res_knownGT_noRoh_200iters.RData')
load(file='./results/R0res_knownGT_noRoh_200iters.RData') # Loads ret_sm, fit, full_data, full_data_stoch
...
summary(exp(extract(fit)$lRs_star))
mean(extract(fit)$R)
...
save(ret_sm, fit_knGT, full_data_knGT, file='./results/R0_res_knownGT_2000iters.RData')
#load(file='./results/R0_res_unknownGT_2000iters.RData') # Loads ret_sm, fit, full_data, full_data_stoch
load(file='./results/R0_res_knownGT_2000iters.RData') # Loads ret_sm, fit_knGT, fit_stoch_knGT, full_data, full_data_stoch
