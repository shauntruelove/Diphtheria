...
mod <- glm(died ~ Vacc, data=CFR.Age.long, family = binomial(link = "logit"))
exp(coef(mod))[2]
exp(cbind("Odds ratio" = coef(mod), confint.default(mod, level = 0.95)))

# Calculate the probabilities
probs1 <- predict(mod, data.frame(Vacc=unique(CFR.Age.long$Vacc)), type = "response")
names(probs1) <- unique(CFR.Age.long$Vacc)
probs1

prob1.se <- predict(mod, data.frame(Vacc=unique(CFR.Age.long$Vacc)), se.fit=TRUE)
...
probs1 <- cbind(exp(prob1.se$fit) / (1+exp(prob1.se$fit)),
                ...
                exp(prob1.se$fit + 1.96*prob1.se$se.fit) / (1+exp(prob1.se$fit + 1.96*prob1.se$se.fit)))
row.names(probs1) <- unique(CFR.Age.long$Vacc)
probs1
...
mod <- glmer(died ~ Group + (1|LocDecade), data=CFR.Age.long, family = poisson(link = "log"))
summary(mod)
exp(fixef(mod))
