...
# Estimate Proportion Reduction of Rs -------------------------------------

delta1day <- calc_delta(delay=1)
delta2day <- calc_delta(delay=2)
delta5day <- calc_delta(delay=5)

# delta1day <- calc_delta(treat.delay=1, time.to.membr=(2.5+1.9))
# delta2day <- calc_delta(treat.delay=2, time.to.membr=(2.5+1.9))
# delta5day <- calc_delta(treat.delay=5, time.to.membr=(2.5+1.9))
...
for (i in 1:nrow(fits95)) {
    Rs_star_scen1 <- Rs_treat(delta=calc_delta(treat.delay=delay[1], time.to.membr=(1.9)), alpha=alpha[1], Rs=fits95$Rs[i])
    Rs_star_scen2 <- Rs_treat(delta=calc_delta(treat.delay=delay[2], time.to.membr=(1.9)), alpha=alpha[2], Rs=fits95$Rs[i])
    Rs_star_scen3 <- Rs_treat(delta=calc_delta(treat.delay=delay[3], time.to.membr=(1.9)), alpha=alpha[3], Rs=fits95$Rs[i])
...
for (i in 1:nrow(fits95)) {
    fits95$PropIsolate[i] <- calc_alpha(p3=fits95$p3[i], Rs=fits95$Rs[i], tau=fits95$tau[i], delta=0.5)
}
